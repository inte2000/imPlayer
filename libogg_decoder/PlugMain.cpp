...
std::string Utf16ToUtf8String(const std::wstring& value)
{
    if (value.empty()) {
        return {};
    }
...
const char8_t* plugname = u8"libogg decoder";
...
typedef struct tagDecoderContext
{
    ...
    std::unique_ptr<OggPlayCtrl> playCtrl;
} DecoderContext;
...
int WINAPI Plug_OnRegister(const ApplicationConfig* app, PluginRegister* regInfo)
{
    ...
    const FileExtRegItem fmtMap[] = {
        {StreamFormatOgg, "Ogg Vorbis", ".ogg;.oga"},
    };
...
uint32_t WINAPI Plug_ParseFileTypeID(const char* filename, CDataStream* pStream)
{
    if ((filename != nullptr) && (filename[0] != '\0')) {
        return ParseStreamFormatByLibogg(filename);
    }
    if (pStream == nullptr) {
    ...
    return ParseStreamFormatByLibogg(streamNameUtf8.c_str());
}
...
void* WINAPI Plug_OnInitialize(const PluginInitialize* init)
{
    ...
    pCtx->playCtrl = std::make_unique<OggPlayCtrl>();

    if (!pCtx->playCtrl->Init(init->pStream, init->streamFmt))
    {
        strcpy_s(errorMsg, "Failed to initialize libogg stream controller.");
        delete pCtx;
        return nullptr;
    }
