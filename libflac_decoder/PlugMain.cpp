...
const char8_t* plugname = u8"libFLAC decoder";
...
typedef struct tagDecoderContext
{
    ...
    std::unique_ptr<FlacPlayCtrl> playCtrl;
} DecoderContext;
...
uint32_t WINAPI Plug_ParseFileTypeID(const char* filename, CDataStream* pStream)
{
    if ((filename != nullptr) && (filename[0] != '\0')) {
        return ParseStreamFormatByLibflac(filename);
    }
    if (pStream == nullptr) {
    ...
    return ParseStreamFormatByLibflac(streamNameUtf8.c_str());
}
...
void* WINAPI Plug_OnInitialize(const PluginInitialize* init)
{
    ...
    pCtx->playCtrl = std::make_unique<FlacPlayCtrl>();

    if (!pCtx->playCtrl->Init(init->pStream, init->streamFmt))
    {
        strcpy_s(errorMsg, "Failed to initialize libFLAC stream controller.");
        delete pCtx;
        return nullptr;
    }
