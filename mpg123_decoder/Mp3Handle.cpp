...
bool CMp3Handle::OpenStream(CDataStream* pStream)
{
    ...
    valid = mpg123_format2(m_hMpg12, 0, MPG123_STEREO | MPG123_MONO, MPG123_ENC_FLOAT_32);
    if(valid != MPG123_OK)
        return false;
#endif
    long rate;
...
/*
template<typename T, std::enable_if<std::is_enum<T>>>
constexpr auto Value(T value) noexcept -> (std::underlying_type<T>) {
