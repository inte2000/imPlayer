...
std::string ReadFfmpegOptionString(const AVFormatContext* fmtCtx, const char* key)
{
    if (fmtCtx == nullptr || key == nullptr) {
        return std::string();
    }

    uint8_t* value = nullptr;
    const int ret = av_opt_get((void *)fmtCtx, key, AV_OPT_SEARCH_CHILDREN, &value);
...
std::string ExtractIcyFieldValue(const std::string& packet, const char* key)
{
    if (packet.empty() || key == nullptr || key[0] == '\0') {
        return std::string();
    }

    const std::string keyText(key);
    const std::string needle = keyText + "='";
    const std::size_t begin = packet.find(needle);
    if (begin == std::string::npos) {
        return std::string();
    }

    const std::size_t valueBegin = begin + needle.size();
    const std::size_t end = packet.find('\'', valueBegin);
    if (end == std::string::npos || end <= valueBegin) {
        return std::string();
    }

    return packet.substr(valueBegin, end - valueBegin);
}
...
bool FfmpegPlayCtrl::Init(CDataStream* stream, uint32_t streamFmt, const PluginConfig& config)
{
    ...
    m_stream = stream;
    m_streamUrl = Utf16ToUtf8(stream->GetName());
    ...
    m_fmtCtx = nullptr;
    if (avformat_open_input(&m_fmtCtx, m_streamUrl.c_str(), nullptr, nullptr) < 0) {
        Release();
        return false;
    }
...
bool FfmpegPlayCtrl::IsCanSeeking() const
{
    ...
    if (m_fmtCtx->pb == nullptr) {
        return false;
    }
    return (m_fmtCtx->pb->seekable & AVIO_SEEKABLE_NORMAL) != 0;
}
...
void FfmpegPlayCtrl::FillMetaTags(CMediaTag& tags) const
{
    ...
    if (m_fmtCtx != nullptr) {
        std::string title = GetIcyTitle();
        if (title.empty()) {
            title = DictValue(m_fmtCtx->metadata, "title");
        }
...
std::string FfmpegPlayCtrl::GetIcyTitle() const
{
    if (m_fmtCtx == nullptr) {
        return std::string();
    }

    std::string title = DictValue(m_fmtCtx->metadata, "StreamTitle");
    if (!title.empty()) {
        return title;
    }

    const std::string icyPacket = ReadFfmpegOptionString(m_fmtCtx, "icy_metadata_packet");
    title = ExtractIcyFieldValue(icyPacket, "StreamTitle");
    if (!title.empty()) {
        return title;
    }

    if (!m_audioStreamIdx.empty()) {
        uint32_t useIndex = (m_activeStreamIdx == static_cast<uint32_t>(-1)) ? 0 : m_activeStreamIdx;
        useIndex = std::min<uint32_t>(useIndex, static_cast<uint32_t>(m_audioStreamIdx.size() - 1));
        AVStream* stream = m_fmtCtx->streams[m_audioStreamIdx[useIndex]];
        if (stream != nullptr) {
            title = DictValue(stream->metadata, "StreamTitle");
        }
    }
