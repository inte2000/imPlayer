...
const char8_t* plugname = u8"Game Music Emu decoder";
...
typedef struct tagDecoderContext
{
    ...
    std::unique_ptr<GmePlayCtrl> playCtrl;
} DecoderContext;
...
static bool EnsurePluginConfigLoaded()
{
    ...
    const std::filesystem::path cfgPath = GetGmePluginConfigPath();
    if (!LoadGmePluginConfigFile(cfgPath, g_pluginCfg)) {
        SaveGmePluginConfigFile(cfgPath, g_pluginCfg);
    }
...
int WINAPI Plug_OnRegister(const ApplicationConfig* app, PluginRegister* regInfo)
{
    ...
    SetGmeCustomFormatBase(FileTypeIdBegin);

    const FileExtRegItem fmtMap[] = {
        { GmeFormatAy(), "AY music", ".ay" },
        { GmeFormatGbs(), "Game Boy Sound", ".gbs" },
        { GmeFormatGym(), "Genesis YM2612 Log", ".gym;.gymx" },
        { GmeFormatHes(), "PC Engine HES", ".hes" },
        { GmeFormatKss(), "MSX KSS", ".kss" },
        { GmeFormatNsf(), "NES Sound Format", ".nsf" },
        { GmeFormatNsfe(), "NES Sound Format Extended", ".nsfe" },
        { GmeFormatSap(), "Atari SAP", ".sap" },
        { GmeFormatSpc(), "Super Nintendo SPC", ".spc" },
        { GmeFormatVgm(), "Video Game Music", ".vgm;.vgz" },
    };
...
uint32_t WINAPI Plug_ParseFileTypeID(const char* filename, CDataStream* pStream)
{
    if ((filename != nullptr) && (filename[0] != '\0')) {
        return ParseStreamFormatByGme(filename);
    }
    if (pStream == nullptr) {
    ...
    return ParseStreamFormatByGme(streamNameUtf8.c_str());
}
...
void* WINAPI Plug_OnInitialize(const PluginInitialize* init)
{
    ...
    pCtx->playCtrl = std::make_unique<GmePlayCtrl>();

    if (!pCtx->playCtrl->Init(init->pStream, init->streamFmt, pCtx->pluginCfg))
    {
        strcpy_s(errorMsg, "Failed to initialize libgme stream controller.");
        delete pCtx;
        return nullptr;
    }
...
int WINAPI Plug_IsSupportOutput(void* ctxhdr, uint32_t mediaStreamIdx, const AudioFormat* audioFmt)
{
    ...
    if ((mediaStreamIdx != static_cast<uint32_t>(-1)) && (mediaStreamIdx >= pCtx->playCtrl->GetStreamCount()))
    {
...
int WINAPI Plug_QueryMetaInfo(void* ctxhdr, uint32_t streamIdx, AudioMetaTags* metaTags)
{
    ...
    pCtx->playCtrl->FillMetaTags(metaTags->tags, streamIdx);
...
void WINAPI Plug_ConfigPlugin(HWND hWnd)
{
    ...
    auto onOk = [&]() {
        ...
        if (!SaveGmePluginConfigFile(GetGmePluginConfigPath(), nextConfig)) {
